...
async def test_retrieve_memories(service):
    # Pre-store a memory
    mem = Memory(
        content="test", context="ctx", metadata={}, timestamp=datetime.now(timezone.utc)
    )
...
async def test_retrieve_memories_error(service):
    service.storage_provider.get_memories_by_context = lambda *args, **kwargs: (
        _ for _ in ()
    ).throw(StorageError("fail"))
...
async def test_store_memories_without_embeddings(service):
    ...
    data = [{"content": "hello", "metadata": {}}, {"content": "world", "metadata": {}}]
    memories = await service.store_memories(
        data, context="ctx", generate_embeddings=False
    )
