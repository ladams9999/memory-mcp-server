...
class MemoryService:
    ...
    async def store_memories(
        ...
        try:
            ...
            logger.info(f"Successfully stored {len(memory_ids)} memories in context '{context}'")
            return memories
    ...
    async def retrieve_memories(
        ...
        try:
            ...
            if metadata_filters:
                logger.warning("Metadata filters are not yet supported by the storage interface")
    ...
    async def search_memories(
        ...
        try:
            ...
            memories = await self.retrieve_memories(context, metadata_filters=metadata_filters)
            ...
            if not memories_with_embeddings:
                logger.warning(f"No memories with embeddings found in context '{context}'")
                return []
            ...
            raise MemoryServiceError(f"Failed to generate query embedding: {e}", cause=e)
        except Exception as e:
    ...
    async def _generate_embeddings_batch(self, memories: List[Memory]) -> None:
        ...
        logger.info(f"Generating embeddings for {len(memories)} memories in batches of {self.batch_size}")
                ...
                try:
                    ...
                    for (memory, _), result in zip(tasks, results):
                        if isinstance(result, Exception):
                            logger.error(f"Failed to generate embedding for memory {memory.id}: {result}")
                    ...
                    raise EmbeddingError(f"Batch embedding failed: {e}", provider="batch")
        
        successful_embeddings = sum(1 for m in memories if m.embedding is not None)
        logger.info(f"Successfully generated {successful_embeddings}/{len(memories)} embeddings")
    ...
    async def get_memory_stats(self, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "embedding_coverage": memories_with_embeddings / total_memories if total_memories > 0 else 0.0,
                "contexts": contexts,
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            if hasattr(self.embedding_provider, 'close') and callable(getattr(self.embedding_provider, 'close')):
